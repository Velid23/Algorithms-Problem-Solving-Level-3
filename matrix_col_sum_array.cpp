...
void printArray(int array[3], int length) {
	for (int i = 0; i < length; i++) {
		cout << "the sum of column " << i + 1 << " is: " << array[i]<<endl;
	}
}
...
void PrintEachRowSum(int sumArray[3],int arr[3][3], short Rows, short Cols)
{
	cout << "\nThe following are the sum of each column in the matrix:\n";
	for (short i = 0; i < Rows; i++)
	{
		sumArray[i]=RowSum(arr, Rows, i);
	}
}
...
int main() {
	...
	PrintEachRowSum(sumcolumn,myMatrix, 3, 3);
	printArray(sumcolumn,3);
